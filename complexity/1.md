...
В (этой ?) серии статей я соберу бОльшую часть своего опыта разработки на Ruby on Rails. Эти методики позволяют контролировать сложность и облегчают сопровождение проекта. Большинство методик (методологий/из них) придумал не я, и, по возможности, я буду указывать источник.  
...
При построении формы я указываю только необходимое: список полей и их тип. Тексты для labels, placeholders, submit подставляются автоматически - достаточно прописать в локалях (локалиях?) правильные ключи:
...
Это self, доступный в хэлперах и вьюхах (презентерах?). Соответственно в презентерах можно использовать все хэлперы.
