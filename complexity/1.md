...
Основная проблема проектов на RoR в том, что, как правило, всю логику пытаются уместить в модели, контроллеры и представления. Т.е. код находится только в моделях(ActiveRecord::Base), контроллерах, хэлперах и шаблонах. Такой подход приводит к печальным последствиям: долго делаются фичи, появляются регрессии, у разработчиков пропадает мотивация. В качестве примера можно посмотреть на исходники [redmine](https://github.com/redmine/redmine/blob/master/app%2Fmodels%2Fissue.rb) или [coderwall](https://github.com/assemblymade/coderwall/blob/master/app%2Fmodels%2Fteam.rb).
...
gem drapper внес путаницу: его разработчики назвали презентеры декораторами. Хотя эти паттерны похожи, они имеют значительное различие: декораторы не изменяют интерфейс. Так же с этим гемом есть много [проблем](https://github.com/drapergem/draper/issues/655) (можно посмотреть на список issues).
...
В этой части я рассмотрел способы организации логики представлений. В следующей я покажу как можно организовать логику контроллеров. В последующих - расскажу про модели. А именно: form-objects, services, ACL, query-objects, взаимодействие с различными храилищами.
