...
Если бизнес тразакция долгая, то простаивает, в том числе, и ответсвенный за соединение процесс в самой БД.
...
тербеуется время. Даже если запрос выполняется 10ms, то 100 запросов будут выполняться секунду.
...
А что если разделить эти 2 обязанности и использвать для состояния
...
`Ref` - контейнер для неизменяемого состояния, допускащий его контролируемую замену.
`Ref` моделирет идентичность подобно тому, как мы даем названия предметам.
Мы называем реку "Волга", но в каждый момент времени она имеет разные неизменяемые состояния.
...
`t.get` и `t.create` возвращают экземляр `Ref`.
...
`t` может запоминать начальное состояние сучщностей,
...
Это оптимиситический и пессимистический подходы к фиксации изменений.
...
Или вставить и обновить при конфликете данные (upsert).

В случае проблем производительностью связку из Storage и Query можно заменить такой командой.

## Связи

Если сущности беспорядочно ссылаются друг на друга, сложно обеспечить согласвованность при их изменении.
...
Агрегат может, например, состоять из Поста (корень) и его переводов. Или Заказ и его Позиции.
...
Я сделал акценты на проблемы при работе с БД, показал вариант применения имутабельности.
